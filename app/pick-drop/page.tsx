    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ background: online ? 'linear-gradient(135deg,#16a34a,#22c55e)' : '#f8fafc', border: online ? 'none' : '2px solid #e2e8f0', borderRadius: 24, padding: '36px 28px', textAlign: 'center' as const }}>
        ...
        <button onClick={online ? goOffline : goOnline} disabled={saving}
          style={{
            background: online ? '#fff' : '#16a34a', color: online ? '#dc2626' : '#fff', border: 'none', borderRadius: 14, padding: '16px 36px', fontSize: 16, fontWeight: 900, cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
...
function CustomerMode({ user }: { user: any }) {
  ...
  // Get user location for map when entering step 1
  const getLocationForMap = () => {
    navigator.geolocation?.getCurrentPosition(
      ...
      () => {
        // Fallback to Delhi center if GPS denied
        setMapCoords({ lat: 28.7041, lng: 77.1025 });
      },
      { enableHighAccuracy: true, timeout: 5000 }
    );
  };
  ...
  useEffect(() => {
    if (step === 1) getLocationForMap();
    if (step === 2) loadRunners();
  }, [step]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div key={s} style={{ display: 'flex', alignItems: 'center', flex: i < 2 ? 1 : 'unset' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{
                ...
                background: step >= i ? '#16a34a' : '#f1f5f9', color: step >= i ? '#fff' : '#94a3b8', border: `2px solid ${step >= i ? '#16a34a' : '#e2e8f0'}`
              }}>{i + 1}</div>
      ...
      {step === 0 && (
        <div>
          ...
          <button disabled={!taskType} onClick={() => setStep(1)}
            style={{
              width: '100%', padding: '15px', border: 'none', borderRadius: 14, fontWeight: 800, fontSize: 15, cursor: taskType ? 'pointer' : 'not-allowed', fontFamily: 'inherit',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button disabled={!address} onClick={() => setStep(2)}
              style={{
                flex: 2, padding: '13px', border: 'none', borderRadius: 12, fontWeight: 800, fontSize: 15, cursor: address ? 'pointer' : 'not-allowed', fontFamily: 'inherit',
      ...
      {step === 2 && (
        <div>
          {/* Active Runners Now Online - Quick View */}
          {runners.length > 0 && (
            <div style={{ marginBottom: 24 }}>
              <div style={{ background: 'linear-gradient(135deg,#16a34a 0%,#22c55e 100%)', borderRadius: 16, padding: '20px', color: '#fff' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                  <span style={{ fontSize: 20 }}>🟢</span>
                  <span style={{ fontWeight: 900, fontSize: 18 }}>Active Runners Now Online</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 10 }}>
                  {runners.slice(0, 4).map((r, i) => (
                    <div key={r.id || i} style={{ background: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: '12px 14px', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)' }}>
                      <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 4 }}>{r.name || `Runner ${i + 1}`}</div>
                      <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.85)', marginBottom: 6 }}>
                        {r.task ? `🛠 ${r.task}` : '🛵 Delivery'}
                      </div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: '#fff' }}>
                        {r.dist !== null && r.dist !== undefined ? `📍 ${r.dist} km` : '📍 Location ready'}
                      </div>
                    </div>
                  ))}
                  {runners.length > 4 && (
                    <div style={{ background: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: '12px 14px', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <div style={{ fontWeight: 800, fontSize: 14, textAlign: 'center' as const }}>
                        +{runners.length - 4} more<br />
                        <span style={{ fontSize: 11 }}>runners online</span>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
            <div>
              ...
              <p style={{ color: '#64748b', fontSize: 13, marginTop: 2 }}>
                {runners.length} runner{runners.length !== 1 ? 's' : ''} online — {myCoords ? 'sorted by distance' : 'all online runners'}
              </p>
            </div>
