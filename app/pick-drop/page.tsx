...
function CustomerMode({ user }: { user:any }) {
  ...
  // Get customer location and load nearby runners
  const findRunners = () => {
    setLoadingR(true);
    navigator.geolocation?.getCurrentPosition(
      async (pos) => {
        ...
        setMyCoords({ lat, lng });
        try {
          const res = await getPb().collection('runners_live').getList(1, 50, { filter:'available=true', sort:'-last_seen' });
          const withDist = res.items.map((r:any) => ({
            ...r,
            dist: (r.lat && r.lng) ? distKm(lat, lng, r.lat, r.lng) : 99,
      ...
      () => {
        // No GPS — still load runners without distance
        getPb().collection('runners_live').getList(1, 50, { filter:'available=true' })
          .then(res => { setRunners(res.items); setLoadingR(false); })
          .catch(() => { setRunners([]); setLoadingR(false); });
      },
      { enableHighAccuracy:true, timeout:8000 }
    );
  };

  useEffect(() => { if (step === 2) findRunners(); }, [step]);
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:0 }}>
        {['Select Task','Task Details','Pick Runner'].map((s,i) => (
      ...
      {/* Step 0: Pick task type */}
      {step === 0 && (
        <div>
          <h3 style={{ fontWeight:800, color:'#0f172a', fontSize:18, marginBottom:16 }}>What do you need?</h3>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:12, marginBottom:24 }}>
            {TASKS.map(t => (
              <button key={t.id} onClick={()=>setTaskType(t.id)} style={{ background: taskType===t.id ? '#f0fdf4' : '#fff', border:`2px solid ${taskType===t.id?'#16a34a':'#e2e8f0'}`, borderRadius:16, padding:'20px 14px', cursor:'pointer', fontFamily:'inherit', textAlign:'center' as const, transition:'all 0.15s' }}>
          ...
          <button disabled={!taskType} onClick={()=>setStep(1)} style={{ width:'100%', padding:'15px', border:'none', borderRadius:14, fontWeight:800, fontSize:15, cursor:taskType?'pointer':'not-allowed', fontFamily:'inherit', background:taskType?'#16a34a':'#e2e8f0', color:taskType?'#fff':'#94a3b8', boxShadow:taskType?'0 4px 16px rgba(22,163,74,0.3)':'none' }}>
            Continue →
          </button>
        </div>
      )}
      ...
      {step === 1 && (
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <div>
            ...
            <input value={address} onChange={e=>setAddress(e.target.value)} placeholder="Enter your full address" style={{ width:'100%', border:'1.5px solid #e2e8f0', borderRadius:10, padding:'12px 14px', fontSize:14, fontFamily:'inherit', outline:'none', boxSizing:'border-box' as const }} />
          </div>
          <div>
            <label style={{ fontSize:13, fontWeight:700, color:'#475569', display:'block', marginBottom:6 }}>Items / Instructions</label>
            <textarea value={items} onChange={e=>setItems(e.target.value)} rows={3} placeholder="e.g. 2kg tomatoes, 1L milk, Paracetamol 500mg..." style={{ width:'100%', border:'1.5px solid #e2e8f0', borderRadius:10, padding:'12px 14px', fontSize:14, fontFamily:'inherit', outline:'none', resize:'none' as const, boxSizing:'border-box' as const }} />
          </div>
          <div>
            <label style={{ fontSize:13, fontWeight:700, color:'#475569', display:'block', marginBottom:6 }}>Budget (₹)</label>
            <input value={budget} onChange={e=>setBudget(e.target.value)} placeholder="e.g. 500" type="number" style={{ width:'100%', border:'1.5px solid #e2e8f0', borderRadius:10, padding:'12px 14px', fontSize:14, fontFamily:'inherit', outline:'none', boxSizing:'border-box' as const }} />
          </div>
          <div style={{ display:'flex', gap:10 }}>
            ...
            <button disabled={!address} onClick={()=>setStep(2)} style={{ flex:2, padding:'13px', border:'none', borderRadius:12, fontWeight:800, fontSize:15, cursor:address?'pointer':'not-allowed', fontFamily:'inherit', background:address?'#16a34a':'#e2e8f0', color:address?'#fff':'#94a3b8' }}>
              Find Nearby Runners →
            </button>
          </div>
        </div>
      ...
      {step === 2 && (
        <div>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
            ...
            <button onClick={findRunners} style={{ background:'#f0fdf4', border:'1px solid #d1fae5', borderRadius:10, padding:'7px 14px', fontSize:13, color:'#16a34a', fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>🔄 Refresh</button>
          </div>

          {loadingR ? (
            <div style={{ textAlign:'center', padding:'40px 0' }}>
              <div className="spinner" style={{ margin:'0 auto 12px' }} />
              <p style={{ color:'#94a3b8', fontSize:14 }}>Finding runners near you…</p>
            </div>
          ) : runners.length === 0 ? (
            <div style={{ textAlign:'center', padding:'40px 20px', background:'#f8fafc', borderRadius:16, border:'1px dashed #e2e8f0' }}>
              ...
              <div style={{ color:'#94a3b8', fontSize:14, marginTop:6 }}>Try again in a few minutes.</div>
            </div>
          ) : (
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              {runners.map((r:any) => (
                <div key={r.id} style={{ background:'#fff', border:'1.5px solid #e2e8f0', borderRadius:18, padding:'18px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, boxShadow:'0 2px 8px rgba(0,0,0,0.05)' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:14 }}>
                    <div style={{ position:'relative' }}>
                      <div style={{ width:52, height:52, borderRadius:'50%', background:'linear-gradient(135deg,#0f172a,#334155)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:24 }}>🛵</div>
                      <div style={{ position:'absolute', bottom:0, right:0, width:14, height:14, background:'#16a34a', borderRadius:'50%', border:'2px solid #fff' }} />
                    </div>
                    <div>
                      ...
                      <div style={{ display:'flex', gap:8, marginTop:6 }}>
                        {r.dist < 50 && <span style={{ background:'#f0fdf4', color:'#16a34a', borderRadius:99, padding:'2px 8px', fontSize:11, fontWeight:700, border:'1px solid #d1fae5' }}>📍 {r.dist} km away</span>}
                        <span style={{ background:'#f0fdf4', color:'#16a34a', borderRadius:99, padding:'2px 8px', fontSize:11, fontWeight:700, border:'1px solid #d1fae5' }}>🟢 Online</span>
                      </div>
                    </div>
                  </div>
                    ...
                    {r.phone && (
                      <>
                        <a href={`tel:${r.phone}`} style={{ textDecoration:'none' }}>
                          <button style={{ background:'#16a34a', color:'#fff', border:'none', borderRadius:10, padding:'10px 20px', fontWeight:800, fontSize:14, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:6 }}>📞 Call</button>
                        </a>
                        <a href={`https://wa.me/91${r.phone}?text=Hi, I need a ${selectedTask?.label || 'delivery'} from ${address}.`} target="_blank" rel="noreferrer" style={{ textDecoration:'none' }}>
                          <button style={{ background:'#dcfce7', color:'#16a34a', border:'1px solid #d1fae5', borderRadius:10, padding:'8px 16px', fontWeight:700, fontSize:13, cursor:'pointer', fontFamily:'inherit', width:'100%' }}>💬 WhatsApp</button>
                        </a>
                      </>
                    )}
                  </div>
          ...
          <button onClick={()=>setStep(1)} style={{ marginTop:16, width:'100%', padding:'12px', border:'1.5px solid #e2e8f0', borderRadius:12, color:'#475569', fontWeight:600, fontSize:14, cursor:'pointer', background:'#fff', fontFamily:'inherit' }}>← Back</button>
        </div>
      )}
    </div>
  );
